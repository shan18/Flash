...
def fetch_image_classification_model(model_path):
    print('Fetching Image Classification data from S3')
    obj = S3_CLIENT.get_object(Bucket=BUCKET_NAME, Key=model_path)
    bytestream = io.BytesIO(obj['Body'].read())
    return torch.jit.load(bytestream)


def fetch_text_classification_data(model_path, metadata_path):
    print('Fetching Text Classification data from S3')
